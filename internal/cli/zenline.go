...
// runZenline launches the interactive Zero TUI with the "zenline" skin: a Zen
...
func runZenline(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	fs := flag.NewFlagSet("zenline", flag.ContinueOnError)
	fs.SetOutput(stderr)
	...
	if *snapshot {
		v := *variant - 1
		if v < 0 || v >= len(zenline.Themes) {
			v = 0
		}
		if *boot >= 0 {
			if _, err := fmt.Fprintln(stdout, zenline.RenderBoot(v, !*light, *boot, *width, *height)); err != nil {
				return 1
			}
			return 0
		}
		...
		if *page == "chat" {
			...
			if *perm {
				cd.Perm = &zenline.Perm{Tool: "edit_file", Risk: "medium", Reason: "writes internal/agent/exec.go and loop.go", Summary: "write"}
			}
			if *stream {
			...
			frame = zenline.RenderChat(cd)
		} else {
			frame = zenline.RenderHome(zenline.HomeData{
				Variant: v, Dark: !*light, Width: *width, Height: *height, Header: hdr,
	...
	return runInteractiveTUIWithSkin(stderr, deps, "zenline")
}
